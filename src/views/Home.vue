<template>
  <main class="h-full flex flex-col items-center justify-center">
    <div
      class="border-2 rounded-lg p-8 border-foam flex flex-col justify-center items-center hover:bg-surface transition-all cursor-pointer"
      @click="startFileUpload"
      :class="
        loading &&
        'hover:cursor-not-allowed hover:border-surface hover:bg-base border-surface'
      "
    >
  ...
  const response: void | AxiosResponse = await axios
    ...
    .catch((reason: AxiosError) => {
      ...
      if (reason.response?.status == 400) {
        statusMessage.value =
          (reason.response?.data as any).detail ||
          "unknown error. tell josef pls :("; // i hate this
      }
